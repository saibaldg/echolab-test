...
def save_score_data(actual_data):
    ...
    with open('./actual_data_%s.csv' % forecast_name, 'w') as file:
...
def get_metrics(forecast_name):
    ...
    actual_data = pd.read_csv('./actual_data_%s.csv' % forecast_name, index_col=0, parse_dates=True)
    forecast = pd.read_csv('./forecast_%s.csv' % forecast_name, index_col=0, parse_dates=True)
    initial_data = pd.read_csv('./initial_data.csv', parse_dates=True) #for visual
    ...
    fig_complete = py.plot(graph_data, filename='./%s_graph.html' % forecast_name, auto_open=False)

    #save metrics as csv
    with open('./metrics_%s.csv' % forecast_name, 'w') as file:
        writer=csv.writer(file, delimiter=',',lineterminator='\n',)
